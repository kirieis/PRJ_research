...
public class PodcastService {
        ...
        public List<PodcastResponse> getAllPublicPodcasts() {
                // Dùng findPublicPodcastsFetch để JOIN FETCH creator + level — tránh N+1
                return podcastRepository.findPublicPodcastsFetch()
                                .stream()
                                .map(this::toResponse)
        ...
        public List<PodcastResponse> getPodcastsByCreator(Long creatorId) {
                // Dùng findByCreatorIdFetch để JOIN FETCH creator + level — tránh N+1
                return podcastRepository.findByCreatorIdFetch(creatorId)
                                .stream()
                                .map(this::toResponse)
                                .collect(Collectors.toList());
        }
        ...
        public List<PodcastResponse> getPodcastsByLevel(Long levelId) {
                // Dùng findByLevelIdFetch để JOIN FETCH creator + level — tránh N+1
                return podcastRepository.findByLevelIdFetch(levelId)
                                .stream()
                                .map(this::toResponse)
                                .collect(Collectors.toList());
        }
        ...
        public PodcastResponse getPodcastById(Long id) {
                // Dùng findByIdFetch để JOIN FETCH creator + level — tránh N+1
                Podcast podcast = podcastRepository.findByIdFetch(id)
